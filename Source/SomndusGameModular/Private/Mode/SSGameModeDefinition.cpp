...
#define LOCTEXT_NAMESPACE "SomnduGame"

#if WITH_EDITOR
EDataValidationResult USSGameModeDefinition::IsDataValid(FDataValidationContext& Context) const
{
...
void USSGameModeDefinition::UpdateAssetBundleData()
{
...
TObjectPtr<const USSPawnData> USSGameModeDefinition::GetDefaultPawnData() const
{
	for (const auto& PawnData : PawnsData)
	{
		if (PawnData->PawnTypeTag == DefaultPawnTag)
		{
			return PawnData;
		}
	}
